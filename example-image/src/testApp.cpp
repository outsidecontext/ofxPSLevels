...
void testApp::setup(){
    ...
    levels.setup(image.width, image.height);
    doDrawInfo = true;
}
...
void testApp::keyPressed(int key){
    float step = .1;
    switch (key) {
        case 'B': levels.brightness += step;
            break;
        case 'b': levels.brightness -= step;
            break;
        case 'C': levels.contrast += step;
            break;
        case 'c': levels.contrast -= step;
            break;
        case 'S': levels.saturation += step;
            break;
        case 's': levels.saturation -= step;
            break;
        case 'G': levels.gamma += step;
            break;
        case 'g': levels.gamma -= step;
            break;
        case ' ': doDrawInfo = !doDrawInfo;
            break;
    }
}
